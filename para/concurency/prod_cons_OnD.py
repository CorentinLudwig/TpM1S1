...
class Buffer:
    ...
    def produce(self, msg_val, msg_type, msg_source):
        with self.verrou:
            while self.nb_elem.value == self.nb_cases:
                self.access[0].wait()
            ...
            print('process %2d    produced %2d (type:%d) in place %2d and the buffer is\t\t %s' %
                (msg_source, msg_val, msg_type, position, self.storage_val[:]))
            self.access[1][msg_type].notify()


    def consume(self, id_cons, msg_type):
        with self.verrou:
            while self.nb_elem.value == 0 or self.storage_type[self.ptr_cons.value] != msg_type:
                self.access[1][msg_type].wait()
...
def consumer(id_cons, nb_times, msg_type, buffer):
    for _ in range(nb_times):
        time.sleep(random.random())
        buffer.consume(id_cons, msg_type)
...
if __name__ == '__main__':
    ...
    for id_cons in range(nb_cons):
        msg_type = id_prod % 2
        cons=Process(target=consumer, args=(id_cons, nb_times_cons, msg_type, buffer))
        cons.start()
